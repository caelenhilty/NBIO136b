...
def expandbin(oldvector, olddt, newdt):
    scalingFactor = newdt/olddt
    newvector = np.zeros(round(len(oldvector) * scalingFactor))
    if scalingFactor > 1: #downfactoring
        for val in newvector:
            sum = 0
            for j in range(round(i * scalingFactor), round((i+1)*scalingFactor)):
                sum += oldvector[j]
            val = sum / round(scalingFactor)
        return newvector
    elif scalingFactor == 1:
        return oldvector
    else:
        for val in oldvector:
            for i in range(round(1/scalingFactor)):
                newvector[i] = val
        return newvector
