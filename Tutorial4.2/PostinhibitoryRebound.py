...
### parameter vectors ###
baselines = np.arange(-200, 201, 20) * 1e-12
steps = np.arange(0, 101, 10) * 1e-12
spikes = np.zeros((len(baselines), len(steps)))

for baseline in baselines:
    for step in steps:
        time, V, spikes[baseline][step] = PIR(baseline, step)


fig = plt.figure()
ax = plt.axes(projection = '3d')
ax.plot3D(baselines, steps, spikes)
plt.show()
